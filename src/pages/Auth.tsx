...
const signUpSchema = z.object({
  ...
  userid: z.string().min(3, 'User ID must be at least 3 characters').regex(/^[a-z0-9_]+$/, 'Only lowercase letters, numbers, and underscores'),
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <>
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="username" name="username" value={formData.username} onChange={handleChange} placeholder="John Doe" className="pl-9 input-dark" />
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="userid" name="userid" value={formData.userid} onChange={handleChange} placeholder="johndoe123" className="pl-9 input-dark" />
                    {userIdAvailable === true && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-success text-xs">✓ Available</span>}
                    {userIdAvailable === false && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-destructive text-xs">✗ Taken</span>}
                  </div>
                  ...
                  {suggestions.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      ...
                      {suggestions.map(s => (
                        <button key={s} type="button" onClick={() => { setFormData(p => ({ ...p, userid: s })); setUserIdAvailable(true); setSuggestions([]); }} className="text-xs text-primary hover:underline">{s}</button>
                      ))}
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="email" name="email" type="email" value={formData.email} onChange={handleChange} placeholder="you@example.com" className="pl-9 input-dark" required />
              </div>
            </div>
              ...
              <>
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="password" name="password" type={showPassword ? 'text' : 'password'} value={formData.password} onChange={handleChange} placeholder="••••••••" className="pl-9 pr-9 input-dark" required />
                ...
                {isSignUp && (
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                      <Input id="confirmPassword" name="confirmPassword" type={showPassword ? 'text' : 'password'} value={formData.confirmPassword} onChange={handleChange} placeholder="••••••••" className="pl-9 input-dark" required />
                    </div>
